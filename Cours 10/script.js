...
function generatePassword() {
  ...
  const passwordLength = parseInt(document.getElementById('display-password-length').value);

  let newPassword = '';

  if (data.length === 0) {
    passwordOutput.innerHTML = 'Générateur de MPD';
    alert('Veuillez sélectionner un critères');
  }
  ...
  setTimeout(() => { generateButton.innerHTML = 'Générer mot de passe'; }, 2000);
}
